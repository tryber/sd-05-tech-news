...
def container(url_page, res_article):
    ...
    shares_count = (
        sel_article.css("div.tec--toolbar__item:nth-child(1)::text")
        .get()
        .strip()
    )
    shares_count = int(shares_count.split()[0])
    summary = (
        sel_article.css(".tec--article__body > p:nth-child(1)::text")
        .get()
        .strip()
    )
...
def scrape(fetcher=fetch_content, pages=1):
    ...
    for page in range(pages):
        ...
        url_artigos.extend(
            selector.css("div.tec--list h3 a::attr(href)").getall()
        )
        url_base += f"?pages={page+1}"
    for url_artigo in url_artigos:
